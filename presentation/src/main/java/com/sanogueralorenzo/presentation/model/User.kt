...
class UserItemMapper @Inject constructor(
    ...
    fun mapToDomain(userItem: UserItem): User = User(
        id = userItem.id,
        name = userItem.name,
...
class AddressItemMapper @Inject constructor(private val mapper: LatLngMapper) {
    ...
    fun mapToPresentation(address: Address): AddressItem = AddressItem(
    ...
    fun mapToDomain(addressItem: AddressItem): Address = Address(
...
class LatLngMapper @Inject constructor() {
    ...
    fun mapToDomain(latLng: LatLng): Geo =
        Geo(lat = latLng.latitude.toString(), lng = latLng.longitude.toString())
}

class CompanyItemMapper @Inject constructor() {

    fun mapToPresentation(company: Company): CompanyItem = CompanyItem(
        name = company.name,
        catchPhrase = company.catchPhrase,
        bs = company.bs
    )

    fun mapToDomain(companyItem: CompanyItem): Company = Company(
